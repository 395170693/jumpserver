...
def auth():
    ...
    count = 0
    while count <3:
        username = input("请输入堡垒机的用户名>>>").strip()
        if len(username) ==0:continue
        password = input("请输入堡垒机的密码>>>").strip()
...
def start_session(argvs):
    ...
    if user:
        ...
        while not exit_flag:
            if user.bind_hosts:
                print('ungroupped hosts（z） hosts (%s)' %len(user.bind_hosts) )
            ...
            if choice == 'z':
                ...
                if choice < len(user.host_groups):
                    print("------ 组: %s ------"  % user.host_groups[choice].name )
...
def log_audit(argvs):
    ...
    name_obj = session.query(models.UserProfile).filter(models.UserProfile.username == name).first()
    if name_obj:
        obj = session.query(models.AuditLog).filter(models.AuditLog.user_id == name_obj.id).all()
        for i in obj:
            print('命令id：%s，主机：%s,ip:%s,执行类型：%s，命令：%s，执行时间：%s' % (i.id,
                                                          session.query(models.Host).filter(models.Host.id == i.bind_host_id).first().hostname,
                                                           session.query(models.Host).filter(
                                                               models.Host.id == i.bind_host_id).first().ip,
                                                          i.action_type,i.cmd,i.date
                                                          ))
    else:
        print('您输入的用户不存在！！！')
    # if source:
